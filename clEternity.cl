...
#if defined GPU
 constant short gbposFromCenter[256 * 2] = { $POSFROMCENTER } ;
...
int put( uchar*  patternCounts, uchar* fUsed,  char* tablero, int* ficha, int posx, int posy)
{
...
void extract( uchar*  patternCounts, uchar* fUsed,  char* tablero, int* ficha, int posx, int posy)
{
		int index = (posx * 16 + posy)*4;
		tablero[index] = -1;
		tablero[index+1] = -1;
